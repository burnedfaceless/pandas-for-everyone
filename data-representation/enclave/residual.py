...
try:
    ...
    cursor.execute("SELECT chlorine_residual FROM well_readings WHERE well_id = 24")
    rows = cursor.fetchall()

    data_list = [float(item[0]) if item[0] is not None else None for item in rows]
    ...
    plt.title('Enclave Well Residual')
    plt.xlabel('Values')
